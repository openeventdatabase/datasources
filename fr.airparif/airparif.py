...
indices = ['vlow','low','medium','high','vhigh','none']
alerts = ['','','.warning','.alert','.alert',None]
niveau = ['très bas','bas','moyen','élevé','très élevé',None]
...
for dep in departements:
  ...
  p = dict(type = 'forecast',
       ...
       name = "%s (O3=%s, NO2=%s, PM10=%s)" % (niveau[indice_max], mesure_o3, mesure_no2, mesure_pm10),
       start = (datetime.date.today()).isoformat()+'T00:00:00',
       stop = (datetime.date.today()+timedelta(days=1)).isoformat()+'T00:00:00',
       source = 'http://www.airparif.asso.fr/en/indices/indice-europeen-departement/dep/'+str(dep),
