  ...
  try:
    ...
    if g is not None:
        ...
        if obs.temp is not None:
          properties['temperature:C']=obs.temp.string('C').replace(' C','')
          properties['temperature:F']=obs.temp.string('F').replace(' F','')
          properties['temperature:K']=obs.temp.string('K').replace(' K','')
        if obs.dewpt is not None:
          properties['dew_point:C']=obs.dewpt.string('C').replace(' C','')
          properties['dew_point:F']=obs.dewpt.string('F').replace(' F','')
          properties['dew_point:K']=obs.dewpt.string('K').replace(' K','')
        if obs.press is not None:
          properties['pressure:hPa']=obs.press.string('HPA').replace(' hPa','')
          properties['pressure:in']=obs.press.string('IN').replace(' inches','')
        if obs.wind_dir is not None:
          properties['wind_dir:compass']=obs.wind_dir.compass()
          properties['wind_dir:degrees']=obs.wind_dir.string().replace(' degrees','')
        if obs.wind_speed is not None:
          properties['wind_speed:kmh']=obs.wind_speed.string('KMH').replace(' km/h','')
          properties['wind_speed:mph']=obs.wind_speed.string('MPH').replace(' mph','')
          properties['wind_speed:ms']=obs.wind_speed.string('MPS').replace(' mps','')
        if obs.wind_gust is not None:
          properties['wind_gust:kmh']=obs.wind_gust.string('KMH')
          properties['wind_gust:mph']=obs.wind_gust.string('MPH').replace(' mph','')
          properties['wind_gust:ms']=obs.wind_gust.string('MPS').replace(' mps','')
